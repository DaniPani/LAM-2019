...
plt.subplot(4,1,1)
f_I, = plt.plot(I_means[index])
plt.xlabel('Pixel')
plt.ylabel('I/Imax')

plt.subplot(4,1,2)
f_Q, = plt.plot(Q_means[index])
plt.xlabel('Pixel')
plt.ylabel('Q/Imax')

plt.subplot(4,1,3)
f_U, = plt.plot(U_means[index])
plt.xlabel('Pixel')
plt.ylabel('U/Imax')

plt.subplot(4,1,4)
f_V, = plt.plot(V_means[index])
plt.xlabel('Pixel')
plt.ylabel('V/Imax')


axis = plt.axes([0.25, .03, 0.50, 0.02])
slider = Slider(axis, 'Rows', 0, rowToInvert, valinit=40, valstep=1)
def update(val):
    index = int(slider.val)
    f_I.set_ydata(I_means[index])
    f_Q.set_ydata(Q_means[index])
    f_U.set_ydata(U_means[index])
    f_V.set_ydata(V_means[index])
