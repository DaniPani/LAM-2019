...
I_mean = np.mean(np.transpose(I_I_max[93:103]), axis=1)
Q_mean = np.mean(np.transpose(Q_I_max[93:103]), axis=1)
U_mean = np.mean(np.transpose(U_I_max[93:103]), axis=1)
V_mean = np.mean(np.transpose(V_I_max[93:103]), axis=1)
...
plt.subplot(4,1,1)
f_I, = plt.plot(I[index])
plt.xlabel('Pixel')
plt.ylabel('I/Imax')

plt.subplot(4,1,2)
f_Q, = plt.plot(Q[index])
plt.xlabel('Pixel')
plt.ylabel('Q/Imax')

plt.subplot(4,1,3)
f_U, = plt.plot(U[index])
plt.xlabel('Pixel')
plt.ylabel('U/Imax')

plt.subplot(4,1,4)
f_V, = plt.plot(V[index])
plt.xlabel('Pixel')
plt.ylabel('V/Imax')


axis = plt.axes([0.25, .03, 0.50, 0.02])
slider = Slider(axis, 'Rows', 0, 139, valinit=40, valstep=1)
def update(val):
    index = int(slider.val)
    f_I.set_ydata(I[index])
    f_Q.set_ydata(Q[index])
    f_U.set_ydata(U[index])
    f_V.set_ydata(V[index])
